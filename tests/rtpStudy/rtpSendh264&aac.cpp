...
void sendAAC(int ret, uint8_t* frame_a, FILE* fd_a, AdtsHeader adtsHeader, int socket_a, int port_a, RtpPacket* rtpPacket_a) {
  while (1)
  {
    //读adts header
    ret = fread(frame_a, 1, 7, fd_a);
    if (ret <= 0)
    {
    ...
    if (parseAdtsHeader(frame_a, &adtsHeader) < 0)
    {
      printf("parse err\n");
      break;
    }
    //读aac data
    ret = fread(frame_a, 1, adtsHeader.aacFrameLength - 7, fd_a);
    if (ret < 0)
    {
      printf("read err\n");
      break;
    ...
    rtpSendAACFrame(socket_a, "127.0.0.1", port_a,
      rtpPacket_a, frame_a, adtsHeader.aacFrameLength - 7);
...
int main(int argc, char* argv[])
{
  ...
  while (std::cin >> port_a) {
    if (getchar() == '\n')
      break;
  }
  std::cout << "port is : " << port_a << std::endl;

  fd_a = fopen(AAC_FILE, "rb");
  if (fd_a == nullptr)
  {
  ...
  while (std::cin >> port_v) {
    if (getchar() == '\n')
      break;
  }
  std::cout << "port is : " << port_v << std::endl;

  fd_v = fopen(H264_FILE_NAME, "rb");
  if (fd_v < 0)
  {
    printf("failed to open %s\n", H264_FILE_NAME);
    return -1;
  ...
  frame_a = (uint8_t*)malloc(5000);
  rtpPacket_a = (RtpPacket*)malloc(5000);

  rtpHeaderInit(rtpPacket_a, 0, 0, 0, RTP_VESION, RTP_PAYLOAD_TYPE_AAC, 1, 0, 0, 0x32411);

  rtpPacket_a = (struct RtpPacket*)malloc(500000);
  frame_a = (uint8_t*)malloc(500000);

  rtpHeaderInit(rtpPacket_a, 0, 0, 0, RTP_VESION, RTP_PAYLOAD_TYPE_H264, 0,
    0, 0, 0x88923423);
  //改成线程，音频和视频分开两个线程来实现同时发送
  std::thread sendaac(sendAAC, ret, frame_a, fd_a, adtsHeader, socket_a, port_a, rtpPacket_a);
  sendaac.join();
  //while (1)
  //{
  //  //读adts header
